...
- **Flow A** — первичная оценка решения кейса.  
- **Flow B** — чат для уточняющих вопросов.  

Скриншоты:  
![Flow A](infra/langflow/flows/Flow_A.png)  
![Flow B](infra/langflow/flows/Flow_B.png)  

JSON-файлы потоков:  
- `infra/langflow/flows/Flow A(2).json`  
...
- 🔹 Вынести расчет оценки в бэкенд (убрать агента-калькулятора из Langflow).  
- 🔹 Оптимизация БД: индексы по `session_id`, `case_id` для ускорения выборок.  
- 🔹 Кэширование результатов — повторные оценки не требуют вызова модели.  
- 🔹 Фронтенд: lazy-loading кейсов, индикаторы загрузки.  
- 🔹 Повышение качества оценивания:  
  - более строгие промпты,  
  - контроль покрытия ключевых слов,  
